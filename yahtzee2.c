...
void randomthrow(int d[NUMDICE])
{
    ...
    for (i = 0; i < NUMDICE; i ++) {
        /* set p[i] to random number between 1 and DICEFACES */
        d[i] = (rand() % MAXTHROW) + 1;
    }
}
...
void makehist(const int d[NUMDICE], int h[MAXTHROW])
{
    ...
    /* make h an array filled with 0 */
    for (i = 0; i < MAXTHROW; i++) {
        h[i] = 0;
    }

    /* looping through d[] and adding to h[] */
    for (i = 0; i < NUMDICE; i++) {
        h[d[i] - 1] += 1;
    }
}
...
bool hists_same(const int h1[MAXTHROW], const int h2[MAXTHROW])
{
    ...
    /* looping through both loops simultaneously and checking both items are the same */
    for (i = 0; i < MAXTHROW; i++) {
        if (h1[i] != h2[i]) {
            return false;
        }
...
bool isfullhouse(const int d[NUMDICE])
{
    ...
    if(histo_has(his, 2) && histo_has(his, 3)) {
        return true;
    }
    return false;
}
...
bool is4ofakind(const int d[NUMDICE])
{
    ...
    if(histo_has(his, 4) && histo_has(his, 1)) {
        return true;
    }
    return false;
}
